  ...
  try {
    ...
    if (ibIds.length > 0) {
      const commissionQuery = `
        ...
        WHERE ith.ib_request_id = ANY($1::int[])
        GROUP BY ith.ib_request_id
      `;
      const commissionResult = await query(commissionQuery, [ibIds]);
      commissionResult.rows.forEach(row => {
        commissionMap[row.ib_request_id] = {
          total_commission: parseFloat(row.total_commission || 0),
          fixed_commission: parseFloat(row.total_commission || 0),
...
router.get('/stats', authenticateAdminToken, async (req, res) => {
  try {
    ...
    // Total IB Balance (simplified - would need actual account balance aggregation)
    const balanceResult = await query(`
      SELECT COALESCE(SUM(ib_commission), 0) as total_balance
      FROM ib_trade_history
      WHERE ib_request_id IN (SELECT id FROM ib_requests WHERE LOWER(TRIM(status)) = 'approved')
    `);
